...
func TestContactsAPI_Contacts(t *testing.T) {
	tests := map[string]struct {
		...
		"HTTP GET /api/v1/admin/contacts response: 200": {
			expectedResponse: testfixtures.ExpectedContactsPageFromAdminAPI(t),
			responder:        httpmock.NewJsonResponderOrPanic(http.StatusOK, httpmock.File("testfixtures/jsons/admin_api/get_contacts_200.json")),
		},
...
func TestContactsAPI_ContactUpdate(t *testing.T) {
	...
	tests := map[string]struct {
		...
		fmt.Sprintf("HTTP PUT /api/v1/admin/contacts/%s response: 200", id): {
			expectedResponse: testfixtures.ExpectedUpdatedUserContactFromAdminAPI(t),
			responder:        httpmock.NewJsonResponderOrPanic(http.StatusOK, httpmock.File("testfixtures/jsons/admin_api/put_contact_update_200.json")),
		},
