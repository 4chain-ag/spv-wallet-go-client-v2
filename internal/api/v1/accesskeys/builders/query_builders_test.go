...
func TestUserAccessKeyFilterQueryBuilder_Build(t *testing.T) {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			// when:
			queryBuilder := builders.UserAccessKeyFilterQueryBuilder{
...
func TestAdminAccessKeyFilterQueryBuilder_Build(t *testing.T) {
	tests := map[string]struct {
		...
		"admin access key filter: filter with only 'xPubId' field set": {
			filter: filter.AdminAccessKeyFilter{
				XpubID: spvwallettest.Ptr("9b496655-616a-48cd-a3f8-89608473a5f1"),
			},
			expectedParams: url.Values{
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			// when:
			queryBuilder := builders.AdminAccessKeyFilterQueryBuilder{AccessKeyFilter: tc.filter}
