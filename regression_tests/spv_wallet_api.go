...
// spvWalletAPIConfig contains configuration settings for initializing a SPVWalletAPI instance.
// These include the environment URL and private keys required for admin and user operations.
type spvWalletAPIConfig struct {
...
// Validate validates the spvWalletAPIConfig.
// It ensures that required fields like EnvURL and keys are not empty.
func (c *spvWalletAPIConfig) Validate() error {
	if c.envURL == "" {
...
// spvWalletAPI represents the core API for interacting with the SPV Wallet ecosystem.
// It holds configuration and client instances for admin, user, and leader operations.
type spvWalletAPI struct {
	cfg    *spvWalletAPIConfig // Configuration for the SPV Wallet API Config (e.g., environment URL, keys).
...
// initSPVWalletAPI initializes the spvWalletAPI with Admin, Leader, and User clients.
// It accepts an sPVWalletAPIConfig and a user alias to be created as input parameters.
...
func initSPVWalletAPI(cfg *spvWalletAPIConfig, alias string) (*spvWalletAPI, error) {
